...
def get_employee_by_session():
    ...
    cur.execute("""
        SELECT
            ...
            u.department,
            u.role,
            u.work_email AS directory_email,
        ...
        LEFT JOIN employee_profile ep ON ep.staff_id = u.staff_id
        WHERE u.staff_id = ?
    """, (staff_id,))
    row = cur.fetchone()
    conn.close()
...
# =========================================================
# DB init + simple "migration" helpers
# =========================================================
def _add_column_if_missing(table, col_name, col_type):
    """
    SQLite doesn't support IF NOT EXISTS for ADD COLUMN in all versions reliably,
    so we check PRAGMA table_info first.
    """
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute(f"PRAGMA table_info({table})")
    cols = [r[1] for r in cur.fetchall()]  # 2nd field is column name
...
def init_db():
    ...
    # Employee profile (extra fields for your employee profile page)
    cur.execute("""
    ...
    # ---- Migrate customers table to include fields used in your HTML ----
    _add_column_if_missing("customers", "home_number", "TEXT")
    _add_column_if_missing("customers", "birth_date", "TEXT")
    _add_column_if_missing("customers", "gender", "TEXT")
    _add_column_if_missing("customers", "postal_code", "TEXT")
    ...
    # Seed employee_profile rows so LEFT JOIN always has something (optional but nice)
    cur.execute("SELECT staff_id FROM users")
    staff_list = [r[0] for r in cur.fetchall()]
    for sid in staff_list:
        cur.execute("SELECT 1 FROM employee_profile WHERE staff_id = ?", (sid,))
        if not cur.fetchone():
            cur.execute("INSERT INTO employee_profile (staff_id) VALUES (?)", (sid,))
...
# ✅ FIX: Customer Edit Profile route
@app.route("/customereditprofile", methods=["GET", "POST"])
def customereditprofile():
    ...
    if request.method == "GET":
        return render_template("customereditprofile.html", customer=customer)
    ...
    cur.execute("""
        UPDATE customers
        SET full_name = ?, email_address = ?, phone_number = ?, home_number = ?,
            birth_date = ?, gender = ?, address = ?, postal_code = ?
        WHERE id = ?
    """, (full_name, email_address, phone_number, home_number, birth_date, gender, address, postal_code, customer["id"]))
    conn.commit()
    conn.close()

    return redirect(url_for("customerprofile", updated="1"))

# ✅ FIX: Customer Change Password route
@app.route("/customerchangepassword", methods=["GET", "POST"])
def customerchangepassword():
    customer = get_customer_by_session()
    if not customer:
        return redirect(url_for("customerloginpage"))

    if request.method == "GET":
        return render_template("customerchangepassword.html", error=None)

    current_pw = request.form.get("current_password", "").strip()
    new_pw = request.form.get("new_password", "").strip()
    confirm_pw = request.form.get("confirm_password", "").strip()

    if current_pw != customer["password"]:
        return render_template("customerchangepassword.html", error="Current password is wrong.")
    if len(new_pw) < 8:
        return render_template("customerchangepassword.html", error="New password must be at least 8 characters.")
    if new_pw != confirm_pw:
        return render_template("customerchangepassword.html", error="New password and confirm password do not match.")
    if new_pw == current_pw:
        return render_template("customerchangepassword.html", error="New password must be different from current password.")

    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute("UPDATE customers SET password = ? WHERE id = ?", (new_pw, customer["id"]))
    conn.commit()
    conn.close()
...
# ✅ FIX: Employee Edit Profile route
@app.route("/employeeeditprofile", methods=["GET", "POST"])
def employeeeditprofile():
    ...
    if request.method == "GET":
        return render_template("employeeeditprofile.html", employee=emp, error=None)

    staff_id = emp["staff_id"]

    work_seat = request.form.get("work_seat", "").strip()
    ...
    cur.execute("SELECT 1 FROM employee_profile WHERE staff_id = ?", (staff_id,))
    if not cur.fetchone():
        cur.execute("INSERT INTO employee_profile (staff_id) VALUES (?)", (staff_id,))

    cur.execute("""
        UPDATE employee_profile
        SET work_seat = ?, work_phone = ?, work_email = ?,
            phone_number = ?, home_number = ?, birth_date = ?, gender = ?, nationality = ?,
            address = ?, postal_code = ?, email_address = ?
        WHERE staff_id = ?
    """, (
        work_seat, work_phone, work_email,
        phone_number, home_number, birth_date, gender, nationality,
        address, postal_code, email_address,
        staff_id
    ))
...
# ✅ FIX: Employee Change Password route
...
def employeechangepassword():
    ...
    if not row:
        conn.close()
        return render_template("employeechangepassword.html", error="Employee not found.")

    if current_pw != row["password"]:
        conn.close()
        return render_template("employeechangepassword.html", error="Current password is wrong.")
    if len(new_pw) < 8:
        conn.close()
        return render_template("employeechangepassword.html", error="New password must be at least 8 characters.")
    if new_pw != confirm_pw:
        conn.close()
        return render_template("employeechangepassword.html", error="New password and confirm password do not match.")
    if new_pw == current_pw:
        conn.close()
        return render_template("employeechangepassword.html", error="New password must be different from current password.")

    cur.execute("UPDATE employee_auth SET password = ? WHERE staff_id = ?", (new_pw, staff_id))
    conn.commit()
    conn.close()
...
# Management login uses MANAGEMENT PASSWORD (mgmt_password)
# =========================================================
...
def _admin_check_management_password(staff_id, mgmt_password):
    ...
    if not row["mgmt_password"]:
        return False, "This admin has no Management Password set yet."
...
def management_userlogin():
    ...
    mgmt_password = request.form.get("password")
...
def api_update_employee(staff_id):
    ...
    cur.execute("""
        ...
        WHERE staff_id = ?
    """, (full_name, department, role, work_email, staff_id))
    conn.commit()
    conn.close()
...
def api_delete_employee(staff_id):
    ...
    cur.execute("DELETE FROM employee_profile WHERE staff_id = ?", (staff_id,))
    conn.commit()
    conn.close()
    return jsonify(success=True)
